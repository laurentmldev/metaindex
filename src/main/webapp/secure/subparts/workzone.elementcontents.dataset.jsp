...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	    	    		...
	    	    		<fieldset  id="content.dataset.<s:property value="#curDataset.datasetId"/>" 
	    	    			...
	    	    			onmouseover="if (editModeActive) { 
	    	    							if (selected_dataset_<s:property value="#curDataset.datasetId" />==false) { 
	    	    						 	...
	    	    						 	document.getElementById('workzone.contents.icon.delete.dataset.<s:property value="#curDataset.datasetId"/>').style.display='block';	    	    						 	                        
	    	    						 	</c:if>
   						    ...
   						    onmouseout="if (editModeActive) { 
   						    				if (selected_dataset_<s:property value="#curDataset.datasetId" />==false) { 
   						    			 	...
   						    			 	<c:if test="${not curDataset.readOnly}" >
	   						    				 document.getElementById('workzone.contents.icon.delete.dataset.<s:property value="#curDataset.datasetId"/>').style.display='none';
 						    				</c:if>
	   						    			 <c:if test="${curDataset.layoutDoDisplayName == false}" >
	    	    		...
	    	    		>
	    	    				<legend >
	    	    					...
	    	    					<td>
	    	    						<c:if test="${not curDataset.readOnly}" >
	    	    							<a  href="#"  id="workzone.contents.icon.delete.dataset.<s:property value="#curDataset.datasetId"/>"
...
<center>
			    	   		...
			    	   		<c:if test="${!curDataset.readOnly}">										    	  
			    	   	 		<s:include value="/secure/subparts/workzone.contents.dropzone.metadata.jsp" >
			        		...
			        		<s:iterator value="#columnsData" var="curMetadata" >				        		 
	   						  	<s:include value="/secure/subparts/workzone.contents.metadata.jsp" >
