...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
			  ...
			  <div id="content.metadata.<s:property value="#curMetadata.metadataId"/>" 
				  ...
				  onmouseover="if (editModeActive) { 
				  					...
				  					<c:if test="${not curMetadata.readOnly}" >
				  						document.getElementById('workzone.contents.icon.delete.metadata.<s:property value="#curMetadata.metadataId"/>').style.display='block';
			  						</c:if>
				  ...
				  onmouseout="if (editModeActive) {
				  					...
				  					<c:if test="${not curMetadata.readOnly}" >		
				  						document.getElementById('workzone.contents.icon.delete.metadata.<s:property value="#curMetadata.metadataId"/>').style.display='none';
				  					</c:if>				  					
		  					 }"
			...
			<c:if test="${not curMetadata.readOnly}" >
				<a  href="#"  id="workzone.contents.icon.delete.metadata.<s:property value="#curMetadata.metadataId"/>"
			  	...
			  	<span class="metadata<s:property value="#curMetadata.layoutSize"/>size" >
		    		...
		    		<c:choose>
		     		<c:when test="${curMetadata.datatypeId eq 1}"><%-- Web Page --%>
						...
						<span id="edit.metadata.<s:property value="#curMetadata.metadataId"/>"
								...
								class="matadataTextEdit" >
									<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.1_WebLink.Edit.jsp" >
												<s:param name="curDataset" value="#curDataset" />
												<s:param name="curMetadata" value="#curMetadata" />
											</s:include>
						</span>
		     		...
		     		<c:when test="${curMetadata.datatypeId eq 4}"><%-- Image --%>
			     			<span id="readonly.metadata.<s:property value="#curMetadata.metadataId"/>"	class="metadaText" >
					      	...
					      	<span  id="edit.metadata.<s:property value="#curMetadata.metadataId"/>" 
								style="display:none"
								class="matadataTextEdit" >
									<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.4_Image.Edit.jsp" >
												<s:param name="curDataset" value="#curDataset" />
												<s:param name="curMetadata" value="#curMetadata" />
											</s:include>
							</span>
		     		...
		     		<c:when test="${curMetadata.datatypeId eq 5}"><%-- LongText --%>
		     			<span id="readonly.metadata.<s:property value="#curMetadata.metadataId"/>"	class="metadaText" >
				      	...
				      	<span  id="edit.metadata.<s:property value="#curMetadata.metadataId"/>" 
							style="display:none"
							class="matadataTextEdit" >
								<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.5_LongText.Edit.jsp" >
											<s:param name="curDataset" value="#curDataset" />
											<s:param name="curMetadata" value="#curMetadata" />
										</s:include>
							</span>
		     		...
		     		<c:when test="${curMetadata.datatypeId eq 6}"><%-- Number --%>
		     			<span id="readonly.metadata.<s:property value="#curMetadata.metadataId"/>"	class="metadaText" >
							<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.6_Number.jsp" >
											<s:param name="curDataset" value="#curDataset" />
											<s:param name="curMetadata" value="#curMetadata" />
										</s:include>
						</span>
				      	<span  id="edit.metadata.<s:property value="#curMetadata.metadataId"/>" 
							style="display:none"
							class="matadataTextEdit" >
								<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.6_Number.Edit.jsp" >
											<s:param name="curDataset" value="#curDataset" />
											<s:param name="curMetadata" value="#curMetadata" />
										</s:include>
							</span>
		     		...
		     		<c:when test="${curMetadata.datatypeId eq 10}"><%-- Tiny Text --%>
		     			<span id="readonly.metadata.<s:property value="#curMetadata.metadataId"/>"	class="metadaText" >
			      		...
			      		<span  id="edit.metadata.<s:property value="#curMetadata.metadataId"/>" 
							style="display:none"
							class="matadataTextEdit" >
								<s:include value="/secure/workzone/contents/metadatatypes/workzone.contents.10_TinyText.Edit.jsp" >
											<s:param name="curDataset" value="#curDataset" />
											<s:param name="curMetadata" value="#curMetadata" />
								</s:include>
						</span>
